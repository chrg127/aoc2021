...
(define (check)
  (memf (lambda (board) (or (check-rows board) (check-cols board))) boards))
...
(define (loop nums)
  (if (or (null? nums) (null? boards))
    ...
    (begin (mark (car nums))
           (let ((board (check)))
             (if (pair? board)
               (cons (car nums) (car board))
               (loop (cdr nums)))))))

(define winning-boards '())
(define winning-nums '())

(define (check-remove num)
  (let ((board (check)))
    (if (pair? board)
      (begin (set! boards (remove (car board) boards))
             (set! winning-boards (cons (car board) winning-boards))
             (set! winning-nums (cons num winning-nums))
             (check-remove num))
      '())))
...
(define (sol2 name)
  (parse name)
  (loop2 drawn)
  (let ((board (car winning-boards)) (num (car winning-nums)))
